...
int main (int nargs, char ** args)
{
  ...
  bool force = nargs > 2 ? atoi(args[2]) : 1;
  ...
  double sum2[DIDAQ_NUM_CHANNELS] = {0};
  int nsamp = 0;
  double sum[DIDAQ_NUM_CHANNELS] = {0};
  for (int i = 0; i < N; i++)
  {
  ...
  for (int j = 0; j < DIDAQ_NUM_CHANNELS; j++)
  {
    double mean  = sum[j]/nsamp;
    double rms = sqrt(sum2[j]/nsamp - mean*mean);
    printf(" CH%d  MEAN/RMS =  %f / %f\n", j,  mean +128, rms);
  }
